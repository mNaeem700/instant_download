...
async function getYouTubeInfo(url) {
  ...
  return {
    ...
    thumbnail: `https://img.youtube.com/vi/${vidId}/maxresdefault.jpg`,
    platform: 'YouTube',
    formats: [{ quality: 'Open in YouTube', url: `https://www.youtube.com/watch?v=${vidId}`, size: '', ext: 'mp4' }]
  };
}
...
app.post('/info', async (req, res) => {
  try {
    ...
    res.json({ success: true, data: { title: 'Video', thumbnail: '', platform: 'Unknown', formats: [{ quality: 'Open Link', url: req.body.url, size: '', ext: 'mp4' }] } });
  }
});

app.post('/download', async (req, res) => {
  const { url: videoUrl } = req.body;
  res.json({ success: true, data: { downloadUrl: videoUrl, filename: 'video.mp4' } });
});
